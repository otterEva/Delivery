...
from app.services.maps_api import (
    get_distances_and_durations,
    get_permutations,
    transform_data,
)
import httpx
...
def calculate_route(data: DeliveryRequestSchema):
    ...
    task_id = httpx.post(
        url=f"https://routing.api.2gis.com/async_matrix/create_task/get_dist_matrix?key={api_key}&version=2.0",
        headers=headers,
        json=data,
    ).json()
    ...
    while True:
        task_status = httpx.get(
            url=f'https://routing.api.2gis.com/async_matrix/result/get_dist_matrix/{task_id["task_id"]}?key={api_key}',
            headers=headers,
        ).json()
        ...
        if (
            task_status["status"] == "TASK_IN_QUEUE"
            or "TASK_IN_PROGRESS"
            or "TASK_CREATED"
        ):
            print(task_status)
